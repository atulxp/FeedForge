...
const platforms: Platform[] = ['instagram', 'facebook', 'linkedin', 'x', 'youtube', 'tiktok', 'threads']
...
export class PostsController {
  ...
  @Get()
  getPosts(@Query('status') status?: PostStatus) {
    if (status && !validStatuses.includes(status)) {
      throw new BadRequestException('Unknown post status')
    }
    return this.store.getPosts(status)
  }
  ...
  createPost(@Body() input: CreatePostInput) {
    ...
    return this.store.createPost(input)
  }

  @Patch(':id/status')
  async updateStatus(@Param('id') id: string, @Body() body: { status: PostStatus }) {
    if (!validStatuses.includes(body.status)) {
      throw new BadRequestException('Unknown post status')
    }
    const post = await this.store.updateStatus(id, body.status)
    if (!post) throw new NotFoundException('Post not found')
    return post
  }
  ...
  async retry(@Param('id') id: string) {
    const post = await this.store.retry(id)
    if (!post) throw new NotFoundException('Post not found')
    return post
  }

  private validateCreate(input: CreatePostInput) {
