...
export class LocalStore implements OnModuleInit {
  ...
  async createPost(userId: string, input: CreatePostInput) {
    ...
    const post: Post = {
      ...
      utmUrl: `https://feedforge.local/listen?utm_source=command-center&utm_campaign=${encodeURIComponent(input.campaign ?? 'general')}&utm_content=${randomUUID()}`,
      createdAt: now,
      updatedAt: now,
  ...
  async connectMockAccount(userId: string, platform: Platform, username?: string): Promise<AccountConnectionResult> {
    ...
    const account: Account = {
      ...
      username: username || (platform === 'reddit' ? 'u/new-feedforge' : `@new-${platform}`),
      status: 'active',
